...
- 后端：Node 24 + Express 4 + PostgreSQL 14+（`pg` 连接池，MVCC 事务）
...
`spent_coins.serial` UNIQUE 约束 + `runImmediateTx`（单连接事务）保证原子性。
...
PostgreSQL 的写入并发由 `runImmediateTx`（单连接事务）串行化——同一时刻只有
一个写事务持有连接。取款 init 和支付都是写事务，高并发下 P99 主要受排队影响。

优化方向（Phase 6 可选）：
- PostgreSQL MVCC 已使读不阻塞写
- 如需更高写并发，可考虑将 spent_coins 检查与 INSERT 合并为单条 SQL
  （INSERT ... ON CONFLICT DO NOTHING + 检查 changes），减少往返
- 更激进的优化：连接池调优、读写分离或分库分表

## 文献参考

- [1] k6 文档 — https://k6.io/docs/
- [2] PostgreSQL 事务隔离 — https://www.postgresql.org/docs/current/transaction-iso.html
