-- 001_init_baseline.sql — Phase 0: initial schema baseline
--
-- This is the canonical baseline. It MUST stay in sync with the legacy
-- schema.sql file (which is kept for backward-compat reference). If you edit
-- one, edit both.
--
-- v5 §二 H1 baseline 特判：if users table already exists (legacy dev DB built
-- from old schema.sql before migrationRunner existed), migrationRunner SKIPS
-- executing this file and just marks version=1 in schema_migrations. So this
-- file only executes on fresh DBs.
--
-- All tables use CREATE TABLE IF NOT EXISTS for idempotency (safe to re-run).
-- No INSERT statements — baseline only creates structure, no seed data.
-- Seed data (admin user, etc.) lives in H3 + later migrations.
